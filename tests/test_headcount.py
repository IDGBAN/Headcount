...
def snapshot(taken_at, followers=(), following=()):
    def accounts(names):
        return [{"username": name, "href": headcount.profile_url(name), "timestamp": None}
                for name in names]
...
class TempDirTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher = mock.patch.object(headcount, "SNAPSHOT_DIR", self.tmp / "snapshots")
        patcher.start()
...
class ClassifyTests(unittest.TestCase):
    ...
    def test_follow_lists(self):
        self.assertEqual(headcount.classify("followers_1.json"), "followers")
        self.assertEqual(headcount.classify("followers_12.json"), "followers")
        self.assertEqual(headcount.classify("following.json"), "following")
        self.assertEqual(headcount.classify("Following.JSON"), "following")

    def test_threads_and_mac_metadata(self):
        self.assertIsNone(headcount.classify("your_instagram_activity/threads/followers.json"))
        self.assertIsNone(headcount.classify("__MACOSX/connections/._followers_1.json"))
        self.assertIsNone(headcount.classify("connections/followers_and_following/._following.json"))
        self.assertEqual(headcount.classify("connections/followers_and_following/following.json"),
                         "following")
    ...
    def test_other_files(self):
        for name in ("following_hashtags.json", "pending_follow_requests.json",
                     ...
                     "liked_posts.json"):
            self.assertIsNone(headcount.classify(name), name)


class ExtractAccountsTests(unittest.TestCase):

    def test_value_format(self):
        accounts = headcount.extract_accounts(followers_file("Alice", "bob.b"))
    ...
    def test_title_and_href_format(self):
        accounts = headcount.extract_accounts(following_file("carol_x"))
...
class BuildSnapshotTests(TempDirTest):
    ...
    def test_zip(self):
        ...
        built = headcount.build_snapshot(path)
    ...
    def test_folder(self):
        ...
        built = headcount.build_snapshot(self.tmp / "export")
        self.assertEqual(headcount.handles(built, "followers"), {"amy"})
        self.assertEqual(headcount.handles(built, "following"), {"cat"})
    ...
    def test_split_follower_files_are_merged(self):
        ...
        built = headcount.build_snapshot(write_zip(self.tmp / "export.zip", files))
        self.assertEqual(headcount.handles(built, "followers"), {"amy", "dan"})
    ...
    def test_missing_list_is_refused(self):
        ...
        with self.assertRaises(SystemExit) as caught:
            headcount.build_snapshot(write_zip(self.tmp / "export.zip", files))
    ...
    def test_nested_zip_is_explained(self):
        ...
        with self.assertRaises(SystemExit) as caught:
            headcount.build_snapshot(outer)
    ...
    def test_bad_paths(self):
        with self.assertRaises(SystemExit) as caught:
            headcount.build_snapshot(self.tmp / "nope.zip")
        self.assertIn("no such file", str(caught.exception))

        broken = self.tmp / "broken.zip"
        broken.write_bytes(b"not a zip")
        with self.assertRaises(SystemExit) as caught:
            headcount.build_snapshot(broken)
...
class StorageTests(TempDirTest):
    ...
    def test_round_trip(self):
        saved = headcount.save_snapshot(snapshot("2026-01-01T10:00:00+00:00", ["amy"], ["ben"]))
        self.assertEqual(saved.name, "2026-01-01_100000.json")
        self.assertEqual(headcount.load_snapshots(), [snapshot("2026-01-01T10:00:00+00:00",
                                                           ["amy"], ["ben"])])
        self.assertEqual(list(headcount.SNAPSHOT_DIR.glob("*.partial")), [])

    def test_same_second_does_not_overwrite(self):
        first = headcount.save_snapshot(snapshot("2026-01-01T10:00:00+00:00", ["amy"]))
        second = headcount.save_snapshot(snapshot("2026-01-01T10:00:00+00:00", ["ben"]))
        self.assertNotEqual(first, second)
        self.assertEqual(len(headcount.load_snapshots()), 2)

    def test_sorted_by_real_time_across_offsets(self):
        # 01:30 at -04:00 is before 01:10 at -05:00, the night the clocks go back
        headcount.save_snapshot(snapshot("2026-11-01T01:10:00-05:00", ["late"]))
        headcount.save_snapshot(snapshot("2026-11-01T01:30:00-04:00", ["early"]))
        order = [s["followers"][0]["username"] for s in headcount.load_snapshots()]
        self.assertEqual(order, ["early", "late"])

    def test_bad_files_are_skipped(self):
        headcount.save_snapshot(snapshot("2026-01-01T10:00:00+00:00", ["amy"]))
        folder = headcount.SNAPSHOT_DIR
        ...
        with redirect_stderr(StringIO()) as err:
            loaded = headcount.load_snapshots()
    ...
    def test_bad_accounts_are_dropped(self):
        ...
        headcount.SNAPSHOT_DIR.mkdir()
        (headcount.SNAPSHOT_DIR / "a.json").write_text(json.dumps(data), encoding="utf-8")
        self.assertEqual(headcount.handles(headcount.load_snapshots()[0], "followers"), {"amy"})
    ...
    def test_accounts_saved_by_older_versions_are_cleaned(self):
        ...
        headcount.SNAPSHOT_DIR.mkdir()
        (headcount.SNAPSHOT_DIR / "old.json").write_text(json.dumps(data), encoding="utf-8")
        loaded = headcount.load_snapshots()[0]
        self.assertEqual(headcount.handles(loaded, "following"), {"cat", "dan"})
        self.assertIn({"username": "dan", "href": "https://www.instagram.com/dan/",
                       "timestamp": 5}, loaded["following"])

    def test_missing_folder(self):
        self.assertEqual(headcount.load_snapshots(), [])


class AnalyseTests(unittest.TestCase):

    def test_empty(self):
        self.assertEqual(headcount.analyse([]), {"snapshot_count": 0})

    def test_single_snapshot(self):
        result = headcount.analyse([snapshot("2026-01-01T00:00:00+00:00", ["amy"], ["ben"])])
        self.assertIsNone(result["since_last"])
        self.assertEqual(result["timeline"], [])
        self.assertEqual(result["totals"], {"followers": 1, "following": 1, "mutuals": 0})

    def test_diff(self):
        result = headcount.analyse([
    ...
    def test_ignore_is_case_and_at_insensitive(self):
        ...
        with mock.patch.object(headcount, "IGNORE", ["@Brand"]):
            result = headcount.analyse(data)
...
class IngestTests(TempDirTest):
    ...
    def test_first_then_change_then_identical(self):
        out = quietly(headcount.ingest, self.export("one.zip", ["amy", "ben"]))
        self.assertIn("first snapshot", out)

        out = quietly(headcount.ingest, self.export("two.zip", ["amy", "cat"]))
        self.assertIn("1 unfollowed you, 1 new follower", out)

        out = quietly(headcount.ingest, self.export("three.zip", ["amy", "cat"]))
        self.assertIn("nothing saved", out)
        self.assertEqual(len(headcount.load_snapshots()), 2)

    def test_large_drop_is_refused(self):
        everyone = [f"user{n}" for n in range(60)]
        quietly(headcount.ingest, self.export("full.zip", everyone))
        with self.assertRaises(SystemExit) as caught:
            quietly(headcount.ingest, self.export("last_year.zip", everyone[:20]))
        self.assertIn("40 of the 60 followers", str(caught.exception))
        self.assertEqual(len(headcount.load_snapshots()), 1)

        with mock.patch.object(headcount, "MAX_FOLLOWER_DROP", 1):
            quietly(headcount.ingest, self.export("last_year.zip", everyone[:20]))
        self.assertEqual(len(headcount.load_snapshots()), 2)

    def test_small_account_drop_is_accepted(self):
        quietly(headcount.ingest, self.export("one.zip", ["amy", "ben", "cat", "dan"]))
        quietly(headcount.ingest, self.export("two.zip", ["amy"]))
        self.assertEqual(len(headcount.load_snapshots()), 2)
    ...
    def test_newest_in_inbox(self):
        ...
        with mock.patch.object(headcount, "EXPORT_INBOX", self.tmp):
            self.assertEqual(headcount.newest_in_inbox(), newer)

    def test_empty_inbox(self):
        with mock.patch.object(headcount, "EXPORT_INBOX", self.tmp / "missing"), \
                self.assertRaises(SystemExit):
            headcount.ingest(None)
...
class ServerTests(TempDirTest):
    ...
    def setUp(self):
        super().setUp()
        self.server = headcount.LocalServer(("127.0.0.1", 0), headcount.Handler)
    ...
    def test_data(self):
        headcount.save_snapshot(snapshot("2026-01-01T00:00:00+00:00", ["amy"], ["ben"]))
    ...
    def test_index(self):
        ...
        self.assertEqual(response.status, 200)
        self.assertIn(b"Headcount", body)
    ...
    def test_analysis_failure_is_a_500(self):
        with mock.patch.object(headcount, "analyse", side_effect=RuntimeError("boom")), \
    ...
    def test_port_in_use(self):
        with self.assertRaises(OSError):
            headcount.LocalServer(("127.0.0.1", self.port), headcount.Handler)


class CommandLineTests(unittest.TestCase):

    def test_help(self):
        self.assertIn("ingest", quietly(headcount.main, ["--help"]))

    def test_unknown_command(self):
        with self.assertRaises(SystemExit) as caught:
            headcount.main(["bogus"])
        self.assertIn("headcount.py ingest", str(caught.exception))
